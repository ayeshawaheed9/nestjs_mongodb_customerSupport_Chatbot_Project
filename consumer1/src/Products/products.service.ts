...
import { DiscountService } from 'src/discountModule/discount.service';
@Injectable()
...
// Method to update a product's stock
  ...
  // Get all products with both original and discounted prices if discount is active
  async getAllProducts(): Promise<Product[]> {
    const products = await this.productModel.find().exec();
    const discountPercentage = this.discountService.getDiscountPercentage();
    ...
    return products.map((product) => {
      ...
      return {
        ...
        discountedPrice: discountPercentage ? discountedPrice : null,
      };
    });
  }
